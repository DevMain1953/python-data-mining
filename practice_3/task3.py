...
def create_bar_chart_from(dataframe):
    content_of_chart = plotly.graph_objs.Bar(x=dataframe.Date[::365],
                                             y=dataframe.Close[::365],
                                             marker_color=dataframe.Close[::365],
                                             # we just need string to add color axis, so don't worry about typo
    ...
    bar_chart = plotly.graph_objs.Figure(data=content_of_chart, layout=layout_of_chart)
    return bar_chart
...
if __name__ == '__main__':
    ...
    create_bar_chart_from(date_and_close).show()
