...
def get_average_values_of_random_samples_by_column_name_in(data_from_csv_file, column_name):
    average_values_of_random_samples = []
    for i in range(300):
        average_values_of_random_samples.append(data_from_csv_file[column_name].sample(100).mean())
    return average_values_of_random_samples
...
def add_histogram_with_legend_to_axes_using(data_from_csv_file, axes):
    ...
    axes[3].legend()
    axes[4].hist(get_average_values_of_random_samples_by_column_name_in(data_from_csv_file, 'bmi'))
    return axes
...
def add_title_with_labels_to_axes(axes):
    ...
    axes[4].set_title('Diagram of central limit theorem', color='black', fontsize=16)
    axes[4].set_xlabel('central limit theorem', color='black', fontsize=16)
    return axes


def display_window_with_histogram_using(data_from_csv_file):
    pandas.set_option('display.max_columns', None)
    window, axes = matplotlib.pyplot.subplots(1, 5, figsize=(15, 4))
